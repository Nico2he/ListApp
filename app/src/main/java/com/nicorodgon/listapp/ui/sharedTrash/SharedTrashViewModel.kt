...
class SharedTrashViewModel : ViewModel() {
    ...
    init {
        viewModelScope.launch(Dispatchers.Main) {
            ...
            if (email != null) {
                DbFirestore.getAllObservableSharedListasDisabled(email).observeForever {
    ...
    //La función emptySharedTrash elimina las listas compartidas que se encuentran en la papelera del usuario
    fun emptySharedTrash() {
        viewModelScope.launch(Dispatchers.IO) {
            if (email != null) {
                DbFirestore.emptySharedTrash(email)
            }
        }
    }
}